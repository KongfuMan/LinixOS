...
// sends an IP packet
static void
net_tx_ip(struct mbuf *m, uint8 proto, uint32 ip_dst)
{
    struct ip *iphdr;
...
net_rx_udp(struct mbuf *m, uint16 len, struct ip *iphdr)
{
    ...
    src_ip = ntohl(iphdr->ip_src);
    src_port = ntohs(udphdr->sport);
    dst_port = ntohs(udphdr->dport);
...
net_rx_ip(struct mbuf *m)
{
    ...
    if (htonl(iphdr->ip_dst) != local_ip)
        goto fail;
    // can only support UDP/TCP in transport layer
